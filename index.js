  ...
  try {
    ...
    if (signeduser) {
      if (await login(pass, signeduser[0].pass)) {
        ...
        if (
          signeduser[0].user_type == userType ||
          signeduser[0].user_type == "Admin"
        ) {
...
app.post("/borrow", async (req, res) => {
  try {
    ...
    console.log(id, req.session.user_In.user_id, x);
    res.redirect("/BorrowBooks");
  } catch (e) {
    res.redirect("/BorrowBooks");
  }
